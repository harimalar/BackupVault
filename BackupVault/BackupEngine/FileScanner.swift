...
/// Result of scanning a directory path relative to the source root.
...
final class FileScanner: @unchecked Sendable {
    ...
    func scan(
        ...
        onDirectory: ((Int, ScannedDirectory) -> Void)? = nil,
    ...
    private func scanOne(
        ...
        skipHidden: Bool,
        exclusions: BackupExclusions? = nil,
        onDirectory: ((Int, ScannedDirectory) -> Void)?,
        onFile: @escaping (Int, ScannedFile) -> Void,
        ...
        while let url = enumerator.nextObject() as? URL, !isCancelled {
            ...
            if resourceValues.isDirectory == true {
                ...
                if !relative.isEmpty {
                    onDirectory?(rootIndex, ScannedDirectory(relativePath: relative, url: url))
                }
                continue
            }
